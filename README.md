...
apps/website/           Astro marketing site + install Worker (rigkit.dev)
apps/docs/              Mintlify docs site (rigkit.dev/docs, docs.rigkit.dev redirect)
...
The install Worker serves `curl -fsSL https://rigkit.dev/install | sh`. It reads GitHub Releases as the source of truth, redirects downloads to the release assets, installs to `~/.rigkit/bin/rig`, and adds that directory to the detected shell profile.
