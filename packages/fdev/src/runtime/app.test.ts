...
describe("runtime HTTP app", () => {
  ...
  test("serves control endpoints through Effect HttpApiBuilder handlers", async () => {
    ...
    try {
      ...
      expect(unauthenticated.status).toBe(401);
      expect(unauthenticated.headers.get("x-fdev-api-version")).toBe(String(RUNTIME_API_VERSION));
      ...
      expect(runtimeResponse.headers.get("x-fdev-protocol-hash")).toBe(RUNTIME_PROTOCOL_HASH);
  ...
  test("serveRuntimeEffect stops the runtime server when its scope closes", async () => {
    const root = mkdtempSync(join(tmpdir(), "fdev-runtime-effect-"));
    let url = "";
      ...
      await Effect.runPromise(Effect.scoped(
        Effect.flatMap(
          serveRuntimeEffect({
            ...
            projectDir: root,
            configPath: join(root, "fdev.config.ts"),
  ...
  test("shutdown resolves the runtime server closed promise", async () => {
    const root = mkdtempSync(join(tmpdir(), "fdev-runtime-shutdown-"));
  ...
  test("projects config-defined operations into the runtime manifest", () => {
    ...
    expect(inputSchema.properties.workspace["x-fdev-input"]).toEqual({
      kind: "workspace",
      workflow: "test",
      resolve: "data",
    });
  ...
  test("rejects config operation ids reserved by host commands", async () => {
    const projectDir = mkdtempSync(join(tmpdir(), "fdev-runtime-reserved-operation-"));
    const configPath = join(projectDir, "fdev.config.ts");
    writeFileSync(
      configPath,
      `
        import { defineConfig, sequence } from "${import.meta.dir}/../../../fdev-engine/src/index.ts";
    ...
    try {
      await expect(loadEngine({ projectDir, configPath }))
        .rejects.toThrow("reserved by the fdev host");
    } finally {
  ...
  test("negotiates run sessions with hello acknowledgements and heartbeats", async () => {
    const projectDir = mkdtempSync(join(tmpdir(), "fdev-runtime-session-"));
    const configPath = join(projectDir, "fdev.config.ts");
    writeFileSync(
      configPath,
      `
  ...
  test("exposes persisted workspace payload as workspace data", async () => {
    const projectDir = mkdtempSync(join(tmpdir(), "fdev-runtime-workspace-data-"));
    const configPath = join(projectDir, "fdev.config.ts");
    writeFileSync(
      configPath,
      `
  ...
  test("reports typed operation validation failures on run events", async () => {
    const projectDir = mkdtempSync(join(tmpdir(), "fdev-runtime-validation-"));
    const configPath = join(projectDir, "fdev.config.ts");
    writeFileSync(
      configPath,
      `
  ...
  test("fails run sessions when host hello lacks required methods or capabilities", async () => {
    const { server, projectDir } = await serveRuntimeFixture("fdev-runtime-required-host-", `
  ...
  test("bridges typed host capability requests over run sessions", async () => {
    const { server, projectDir } = await serveRuntimeFixture("fdev-runtime-capability-", `
  ...
  test("resolves host capability resource lifetimes from session close reports", async () => {
    const { server, projectDir } = await serveRuntimeFixture("fdev-runtime-capability-close-", `
  ...
  test("keeps host-owned capability runs attached until the host cancels", async () => {
    const { server, projectDir } = await serveRuntimeFixture("fdev-runtime-capability-attached-", `
  ...
  test("turns host response errors into typed host request failures", async () => {
    const { server, projectDir } = await serveRuntimeFixture("fdev-runtime-capability-error-", `
  ...
  test("turns run.cancel session messages into run failures", async () => {
    const { server, projectDir } = await serveRuntimeFixture("fdev-runtime-cancel-", `
