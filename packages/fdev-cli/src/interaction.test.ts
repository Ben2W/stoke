...
describe("local interaction presenter", () => {
  test("presents provider-owned URLs without waiting for completion", async () => {
    const previousNoBrowser = process.env.FDEV_NO_BROWSER;
    process.env.FDEV_NO_BROWSER = "1";
    ...
    try {
      ...
      if (previousNoBrowser === undefined) {
        delete process.env.FDEV_NO_BROWSER;
      } else {
        process.env.FDEV_NO_BROWSER = previousNoBrowser;
      }
    }
  });
});
