...
export class RuntimeApiVersionError extends Error {
  ...
  constructor(input: { version: string | null; supportedVersion: number }) {
    super(
      input.version
        ? `Unsupported fdev runtime API version ${input.version}; this host supports ${input.supportedVersion}`
        : `Runtime did not report x-fdev-api-version; this host requires ${input.supportedVersion}`,
    );
