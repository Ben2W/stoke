import { Activity, Box, CircleGauge, FolderKanban, Settings2 } from "lucide-react";

const navigation = [
  { label: "Projects", icon: FolderKanban, active: true },
  { label: "Checkouts", icon: Box },
      ...
      <nav className="space-y-1" aria-label="Dashboard navigation">
        {navigation.map(({ label, icon: Icon, active }) => (
          <span
            ...
            className={`flex h-10 items-center gap-3 rounded-md px-3 text-sm ${active ? "bg-zinc-200/70 font-medium text-zinc-950" : "text-zinc-400"}`}
            key={label}
          >
