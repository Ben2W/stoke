...
import { RunLogsDialog } from "../../../../runs/_components/run-logs-dialog.tsx";
...
export function WorkspaceRunHistory({ className = "mt-8", projectId, workspace }: { className?: string; projectId: string; workspace: ManagedWorkspace }) {
  ...
  const runs = useMemo(
    () => (runsResult.data ?? []).filter((run) => run.projectId === projectId && run.workspace === workspace.name),
    [projectId, runsResult.data, workspace.name],
  );
