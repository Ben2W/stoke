...
function renderBashUsage(request: Request, env: Env, url: URL) {
  ...
  return [
    ...
    "",
    "Tip: /monorepo/examples/freestyle-website-next is the recommended reference —",
    "a complete, well-structured Rigkit setup. Read it first and mirror its layout",
    "when scaffolding a config:",
    `  curl ${baseUrl} --data-binary 'find /monorepo/examples/freestyle-website-next -type f'`,
    `  curl ${baseUrl} --data-binary 'cat /monorepo/examples/freestyle-website-next/rigkit/index.ts'`,
    "",
    "Examples:",
    `  curl ${baseUrl} --data-binary 'ls /docs'`,
    `  curl ${baseUrl} --data-binary 'ls /monorepo'`,
    `  curl ${baseUrl} --data-binary 'cat /docs/guides/quickstart.md'`,
    `  curl ${baseUrl} --data-binary 'cat /monorepo/package.json'`,
    `  curl ${baseUrl} --data-binary 'grep -Rni "workspace" /docs | head -20'`,
    `  curl ${baseUrl} --data-binary 'grep -Rni "createDocsVirtualFiles" /monorepo/apps/docs/src | head -20'`,
    `  curl ${baseUrl} --data-binary 'ls /monorepo/examples/freestyle-website-next/rigkit/tasks'`,
    `  curl ${baseUrl} --data-binary 'cat /monorepo/examples/freestyle-website-next/rigkit/index.ts'`,
    ...
    "Filesystem:",
    "  /docs              Markdown docs",
    "  /monorepo          Rigkit monorepo source snapshot",
    "  /monorepo/examples/freestyle-website-next",
    "                     Well-structured Rigkit example — start here",
