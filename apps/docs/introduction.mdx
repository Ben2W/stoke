...
Start with [Quickstart](/guides/quickstart) if you want a working project.
Read [Concepts](/concepts/mental-model) when you want the mental model behind
tasks, context, providers, caches, and workspaces.

When you are ready to write real project configs, use:

- [Workflows](/guides/workflows) for `workflow`, `sequence`, `.task`,
  `.parallel`, `.configure`, and output validation.
- [Workspaces and operations](/guides/workspaces) for `rig create`,
  `rig run`, lifecycle hooks, and operation inputs.
- [Providers](/providers/overview) for Freestyle, terminal sessions, cmux,
  gcloud, custom providers, and provider storage boundaries.
- [Advanced fragments](/advanced/fragments) for reusable global bases and
  wrapper fragments.
- [Creating providers](/providers/creating-providers) for building your own
  provider package.
