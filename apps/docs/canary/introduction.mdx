...
Instead of writing a setup checklist, you write a `rigkit/index.ts` file. Rigkit
...
```mermaid
flowchart LR
  Config["Define environment once<br/>rigkit/index.ts"] --> Apply["Prepare and cache setup<br/>rig apply"]
...
export const website = app
  ...
  .task("install-dependencies", async ({ providers }) => {
    ...
    try {
      const install = await vm.exec("corepack enable");
...
Start with [Quickstart](/canary/guides/quickstart) if you want a working project.
Read [Concepts](/canary/concepts/mental-model) when you want the mental model behind
tasks, context, providers, caches, and workspaces.

When you are ready to write real project configs, use:

- [Workflows](/canary/guides/workflows) for `workflow`, `sequence`, `.task`,
  `.parallel`, `.configure`, and output validation.
- [Workspaces and operations](/canary/guides/workspaces) for `rig create`,
  `rig run`, lifecycle hooks, and operation inputs.
- [Providers](/canary/providers/overview) for Freestyle, terminal sessions, cmux,
  gcloud, custom providers, and provider storage boundaries.
- [Advanced fragments](/canary/advanced/fragments) for reusable global bases and
  wrapper fragments.
- [Creating providers](/canary/providers/creating-providers) for building your own
  provider package.
