...
The same pattern works well for credentials that expire. For instance, you might
set up a `gcloud` CLI with a long-lived cache, but credentials expire after 24
hours. A refresh task can check and reauthenticate cheaply:

```ts
.task(
  "setup-gcloud",
  { cacheTTL: "7d" },
  async ({ freestyle, terminal }) => {
    const { vm, vmId } = await freestyle.client.vms.create({
      logger: console.log,
    });
    ...
    try {
      ...
      await terminal.open("Authenticate with gcloud", {
...
.task(
  ...
  async ({ step, freestyle, terminal }) => {
    ...
    try {
      ...
      const authStatus = await vm.exec("gcloud auth application-default print-access-token");
      if (!authStatus.ok) {
        // Credentials expired, reauthenticate
        await terminal.open("Refresh gcloud authentication", {
          ssh: await freestyle.createSSHOptions({ vmId }),
          command: "gcloud auth login",
          keepOpenAfterCommand: true,
          instructions:
...
When credentials expire, `refresh-gcloud-auth` detects it and opens an
interactive terminal to reauthenticate. The setup remains cached; only the
